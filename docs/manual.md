...
左端子文字と右端子文字は1行1点で入力します。見本PDFのように、箱の左側と右側に別々の端子番号や信号名を出したい場合に使います。入力した端子文字はSVG表示とDXF出力に反映されます。
...
既存配線の上に接点、コイル、ヒューズ、遮断器、ランプ、ブザーなどを配置すると、部品の左右端子位置で配線が自動的に分割されます。部品の中を配線が突き抜けた表示になりにくく、参照図面のようなインライン記号として配置できます。
