...
「2面下絵ページ」を押すと、1 枚の画像から A4 縦ページを 2 枚作成します。1 ページ目には画像の上半分、2 ページ目には画像の下半分が下絵として自動配置されます。
...
端子、接点、コイル、ヒューズ、遮断器、ランプ、ブザー、接地、モータ、ファン、抵抗、ダイオードなどの主要シンボルでは、「見本サイズに戻す」を押すと見本PDF基準の既定寸法に戻せます。
...
「横向き標準」は、見本PDFの横配線上にある小型の接点記号に合わせた形です。新しく配置するa接点、b接点、押しボタンはこの形が既定です。

「縦配線上の回転配置用」は、縦配線上へ回転して配置したい場合の形です。見本PDFに合わせる場合も、まず横向き標準で配置し、必要に応じて右パネルの回転や接点方向を調整します。

ヒューズ、遮断器、コイル、ランプ、ブザー、接地、モータ、抵抗、ダイオードは、見本PDFの部品密度に合わせて新規配置時の既定寸法を小さめにしています。これらの主要シンボルは配線上へ置きやすいインライン記号として表示され、DXF出力でも同じ線分構成で出力されます。

ランプを選択すると、右パネルの「ランプ形状」で「丸形パイロット」と「筒形表示灯」を切り替えられます。ブザーを選択すると、「ブザー形状」で「ホーン」と「スピーカー」を切り替えられます。見本PDFの上部にある筒形表示灯やホーン形ブザーをなぞる場合は、この形状を切り替えてから回転や位置を合わせます。

単体端子は、新規配置時は見本PDFで多い丸端子として配置されます。右パネルの「端子形状」で「丸」と「角」を切り替えられます。
...
左パネルの「縦配線列」を押すと、複数の縦配線を一括生成できます。本数、開始X、上Y、下Y、ピッチ、線番接頭辞、線番開始番号を指定します。見本スキャンの端子台へ伸びる縦配線を先に並べると、部品配置の基準にできます。
...
方向は「横」と「縦」を選べます。見本スキャンのような入出力端子列や縦方向のコネクタを作る場合は「縦」を選びます。
