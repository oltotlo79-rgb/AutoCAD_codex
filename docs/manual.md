...
図枠の下辺と左辺には**座標目盛**が付きます。目盛は太グリッド（10mm）と同じ位置に打たれ、
数字は「mm座標÷10」です（例: 目盛 `12` = 120mm）。右パネルのX/Y座標と直接対応するので、
部品の位置合わせの目安に使えます。目盛は表題欄と重なる範囲には表示されません。
