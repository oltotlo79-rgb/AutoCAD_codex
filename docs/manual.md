...
- 機能メニューの「シンボルビルダー（マウス作図）」: **方眼キャンバスにマウスで直接作図**して
  オリジナル部品を作成できます。直線・矩形・円はドラッグ、文字・接続点（赤丸=配線が吸着）はクリックで配置。
  選択ツールでプリミティブをつかんで移動、削除ツールまたはDeleteキーで削除できます。
  スナップは1／0.5／0.25mmから選択。
  キャンバスの内容は下段の**スクリプト定義**（`line/rect/circle/text/anchor`）と常に同期しており、
