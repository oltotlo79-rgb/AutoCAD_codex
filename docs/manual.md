...
「縦配線用（見本PDF風）」は、見本PDFの縦配線上にある小型の接点記号に合わせた形です。新しく配置するa接点、b接点、押しボタンはこの形が既定です。

「横ラダー用」は、ラダー回路の横配線上に置く従来の横向き接点です。ラダーページのサンプル回路では自動的に横ラダー用が使われます。

ヒューズ、遮断器、コイル、ランプは、見本PDFの部品密度に合わせて新規配置時の既定寸法を小さめにしています。ヒューズと遮断器は配線上へ置きやすいインライン記号として表示され、DXF出力でも同じ線分構成で出力されます。
