...
test("Y座標・文字配置・線の接続点・ページ設定を編集できる", async ({ page }) => {
  ...
  // 選択中要素のミニツールバーが他要素へ重ならないよう、いったん選択解除してから線を選ぶ
  ...
  // 部品選択中は右パネルが「選択部品」タブになるため、ページ設定タブへ切り替えてから操作する
  await page.locator("#inspectorTabPage").click();
...
test("ACE拡張8機能を管理・生成・同期できる", async ({ page }) => {
  ...
  expect(avoided).toEqual({ x: 31, y: 43.2, anchor: "start" });
});
...
test("schema v3のCPとユニット列を接続を保って現行版へ移行し再適用しても変化しない", async ({ page }) => {
  const result = await page.evaluate(() => {
...
test("schema v4のブザーを配線ごとJIS記号の新ピンへ移行し再適用しても変化しない", async ({ page }) => {
  const result = await page.evaluate(() => {
    const buzzer = (id, variant, x, y, extra = {}) =>
      ({ id, type: "buzzer", x, y, w: 12, h: 6, buzzerVariant: variant, label: "BZ", ...extra });
    const source = {
      schemaVersion: 4,
      activePageId: "p1",
      pages: [{
        id: "p1", name: "P1", size: "A4", orientation: "portrait", frameVariant: "blank", title: {},
        elements: [
          // 旧jis(称: 08-10-10)は下向き端子線[3.8,6]/[6.2,6] → 新08-10-06の[3.75,6]/[6.25,6]
          buzzer("bz-jis", "jis", 20, 20),
          { id: "w-jis-l", type: "wire", points: [[24.8, 26], [24.8, 40]] },
          { id: "w-jis-r", type: "wire", points: [[27.2, 26], [27.2, 40]] },
          // 旧general(称: 08-10-06)は左右水平リード → 廃止しjis0806へ統合、配線は下ピンへ
          buzzer("bz-general", "general", 50, 20),
          { id: "w-gen-l", type: "wire", points: [[51, 23.8], [40, 23.8]] },
          { id: "j-gen-r", type: "junction", x: 61, y: 23.8 },
          // 旧siren(横向き三角形)は左右水平リード → 直角二等辺三角形+下向き端子線
          buzzer("bz-siren", "siren", 80, 20),
          { id: "w-siren-l", type: "wire", points: [[81, 24], [70, 24]] },
          { id: "w-siren-r", type: "wire", points: [[91, 24], [100, 24]] },
          // 略画(horn)は規格外のため移行対象外
          buzzer("bz-horn", "horn", 110, 20)
        ]
      }]
    };
    const migrated = window.__edsTest.migrateProjectData(source);
    ...
    return {
      ...
      pins: { jis: pins("bz-jis"), general: pins("bz-general"), siren: pins("bz-siren") }
    };
  });
  ...
  expect(elements["bz-jis"].buzzerVariant).toBe("jis0806");
  expect(elements["bz-general"].buzzerVariant).toBe("jis0806");
  expect(elements["bz-siren"].buzzerVariant).toBe("siren");
  expect(elements["bz-horn"].buzzerVariant).toBe("horn");

  // 3種とも下向き端子線・ピッチ2.5mmの接続点になる
  for (const [name, pair] of Object.entries(result.pins)) {
    expect(pair[1][0] - pair[0][0], name).toBeCloseTo(2.5, 6);
    expect(pair[0][1], name).toBe(pair[1][1]);
  }
  expect(result.pins.jis).toEqual([[24.75, 26], [27.25, 26]]);
  expect(result.pins.general).toEqual([[54.75, 26], [57.25, 26]]);
  expect(result.pins.siren).toEqual([[84.75, 26], [87.25, 26]]);

  // 旧ピンへ接続していた配線・接続点が新ピンへ追従する(反対側の端点は動かさない)
  expect(elements["w-jis-l"].points).toEqual([[24.75, 26], [24.8, 40]]);
  expect(elements["w-jis-r"].points).toEqual([[27.25, 26], [27.2, 40]]);
  expect(elements["w-gen-l"].points).toEqual([[54.75, 26], [40, 23.8]]);
...
test("端子台・PLCユニット列はSVG・吸着点・DXFで同じ端子円中心を使う", async ({ page }) => {
  const result = await page.evaluate(() => {
    const strip = {
      ...
      stripStyle: "unit",
      terminalLabels: "1\n2",
      // このテストは端子円中心の一致検証が目的のため、脇の端子記号(既定=左ON)とCOMポート(既定=あり)は出さない
      terminalMarkLeft: false,
      comTerminal: ""
    };
    const stripTwo = {
...
test("テンプレートは現行の変圧器・雌ソケットと非常停止の接点間隔を保持する", async ({ page }) => {
  await page.locator("#templateMenu").selectOption("acPower");
  await page.locator("#acBuildBtn").click();
  ...
  expect(acSymbols.map(item => item.variant)).toEqual(["coils", "socketPin", "socketPin"]);
  ...
  })).toEqual(["coils", "socketPin", "socketPin"]);
...
test("端子接続図・PLC入出力・汎用ラダーは端子の重ね順と母線表示を保つ", async ({ page }) => {
  ...
  // 入力回路テンプレートは全行に左端子記号があるため、接続点は左辺(125)の端子のみ
  expect([result.unitX, result.unitW, result.firstAnchor.x]).toEqual([125, 10, 125]);
  expect(result.duplicateSmallTerminals).toBe(0);
...
test("ユニット列の端子台は長方形(上下も直線・波形なし)で描かれる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const unit = {
    ...
    const group = document.querySelector('g[data-id="wave-unit"]');
    return {
      pathCount: group.querySelectorAll("path").length,
      lines: Array.from(group.querySelectorAll("line")).map(node =>
        ["x1", "y1", "x2", "y2"].map(attr => Number(node.getAttribute(attr)))),
      dxf: window.__edsTest.buildDxf(current)
    };
  });
  // 波形破断線(path)は廃止。上下も直線の閉じた長方形になる
  expect(result.pathCount).toBe(0);
  expect(result.lines).toEqual(expect.arrayContaining([
    [0, 0, 0, 9.5], [10, 0, 10, 9.5], [0, 0, 10, 0], [0, 9.5, 10, 9.5]
  ]));
  ...
  // DXFも長方形の4辺(上20,277→30,277 / 下20,267.5→30,267.5 / 左右)が直線で出る
  ...
  expect(rectSegments.every(segment => actualKeys.has(key(segment)))).toBe(true);
});
...
test("機器箱の端子記号を端子文字と連動でき、全要素の文字サイズと太字を変更できる", async ({ page }) => {
  ...
  // 端子ごとのチェックは折りたたみ内へ移動したため開いてから操作する(開閉状態は記憶される)
  await page.locator('#selectionPanel details summary', { hasText: "端子ごとの表示" }).click();
...
test("線番は自動退避せず、指定時だけ引き出し線と任意位置を使用する", async ({ page }) => {
  await page.evaluate(() => window.__edsTest.installProjectData({
    schemaVersion: 4, activePageId: "p1",
    pages: [{ id: "p1", name: "P1", size: "A4", orientation: "portrait", frameVariant: "blank", title: {}, elements: [
      { id: "wire", type: "wire", points: [[20, 40], [60, 40]], wireNo: "W100", wireNoAlign: "mid", layer: "wires" },
      { id: "symbol", type: "rect", x: 35, y: 35, w: 20, h: 10, layer: "symbols" }
    ] }]
  }));
  await expect(page.locator('[data-id="wire"] [data-wire-no-leader]')).toHaveCount(0);
  const original = await page.locator('[data-id="wire"] text').first().evaluate(node => ({ x: node.getAttribute("x"), y: node.getAttribute("y") }));

  await page.evaluate(() => window.__edsTest.selectElement("wire"));
  await expect(page.locator('[data-bind="wireNoLeader"]')).not.toBeChecked();
  await page.locator('[data-bind="wireNoLeader"]').check();
  await page.locator('[data-bind="wireNoDx"]').fill("12.5");
  await page.locator('[data-bind="wireNoDx"]').press("Enter");
  await page.locator('[data-bind="wireNoDy"]').fill("6");
  await page.locator('[data-bind="wireNoDy"]').press("Enter");

  await expect(page.locator('[data-id="wire"] [data-wire-no-leader]')).toHaveCount(1);
  const moved = await page.locator('[data-id="wire"] text').first().evaluate(node => ({ x: node.getAttribute("x"), y: node.getAttribute("y") }));
  expect(Number(moved.x) - Number(original.x)).toBeCloseTo(12.5, 3);
  expect(Number(moved.y) - Number(original.y)).toBeCloseTo(-6, 3);
});

test("未保存変更があるときだけ再読み込み前の離脱確認を要求する", async ({ page }) => {
  const clean = await page.evaluate(() => {
    const event = new Event("beforeunload", { cancelable: true });
    window.dispatchEvent(event);
    return { prevented: event.defaultPrevented, dirty: window.__edsTest.isProjectDirty() };
  });
  expect(clean).toEqual({ prevented: false, dirty: false });

  const dirty = await page.evaluate(() => {
    window.__edsTest.state.project.title = "編集中の図面";
    window.__edsTest.commitHistory("図面を編集しました。");
    const event = new Event("beforeunload", { cancelable: true });
    window.dispatchEvent(event);
    return { prevented: event.defaultPrevented, dirty: window.__edsTest.isProjectDirty() };
  });
  expect(dirty).toEqual({ prevented: true, dirty: true });
});

test("光電・マット・リード・ライトカーテンを複数表現で配置し、新規ラベルを空欄にする", async ({ page }) => {
  for (const type of ["photoelectricSwitch", "matSwitch", "reedSwitch", "lightCurtain"]) {
    await expect(page.locator(`#paletteGrid .palette-item[data-type="${type}"]`)).toHaveCount(1);
  }
  const result = await page.evaluate(() => {
    const variants = {
      ...
      sensor: ["ultrasonic", "laser", "hall"]
    };
    const checks = {};
    Object.entries(variants).forEach(([type, values]) => {
      const signatures = values.map(symbolVariant => {
        const element = window.__edsTest.defaultElement(type, 20, 20);
        element.symbolVariant = symbolVariant;
        const prims = window.__edsTest.geoPrims(element);
        const anchors = window.__edsTest.elementConnectionAnchors(element);
        if (!prims.length || anchors.length !== 2) throw new Error(`${type}/${symbolVariant} has invalid geometry`);
        return JSON.stringify(prims);
      });
      checks[type] = { count: values.length, unique: new Set(signatures).size };
    });
    const blankLabels = ["coil", "sensor", "photoelectricSwitch", "matSwitch", "reedSwitch", "lightCurtain"]
      .map(type => window.__edsTest.defaultElementAtPlacement(type, 50, 50).label);
...
test("サーマルリレー接点を複数デザインへ拡張し、設備標準表記を使わない", async ({ page }) => {
  const result = await page.evaluate(() => {
    ...
    const signatures = values.map(symbolVariant => {
      const element = window.__edsTest.defaultElement("thermalRelay", 20, 20);
      element.symbolVariant = symbolVariant;
      const prims = window.__edsTest.geoPrims(element);
      const anchors = window.__edsTest.elementConnectionAnchors(element);
      if (!prims.length) throw new Error(`thermalRelay/${symbolVariant} has no geometry`);
      const xs = anchors.map(anchor => anchor.x);
      return {
        signature: JSON.stringify(prims),
        anchorCount: anchors.length,
        span: Math.max(...xs) - Math.min(...xs)
      };
    });
    const element = { ...window.__edsTest.defaultElement("thermalRelay", 40, 60), id: "thr-variants" };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "thr-page",
      pages: [{ id: "thr-page", name: "サーマル", size: "A4", orientation: "portrait", title: {}, elements: [element] }]
    });
    window.__edsTest.selectElement("thr-variants");
    const labels = Array.from(document.querySelectorAll('select[data-bind="symbolVariant"] option')).map(option => option.textContent);
    return {
      unique: new Set(signatures.map(item => item.signature)).size,
      anchorCounts: signatures.map(item => item.anchorCount),
      spans: signatures.map(item => item.span),
      labels
    };
  });
  expect(result.unique).toBe(10);
  expect(result.anchorCounts).toEqual([2, 2, 2, 2, 2, 2, 2, 2, 3, 2]);
  expect(result.spans).toEqual([10, 10, 10, 10, 10, 10, 10, 10, 10, 10]);
  expect(result.labels).toHaveLength(10);
  expect(result.labels.some(label => label.includes("設備標準"))).toBe(false);
});

test("接触器主接点・トライアック・JISブザー・交流電源のデザインを作画できる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const cases = [
      ["contactNO", { symbolVariant: "contactor" }],
      ["diode", { symbolVariant: "triac" }],
      ["buzzer", { buzzerVariant: "jis0806" }],
      ["powerSupply", { symbolVariant: "ac" }]
    ];
    return cases.map(([type, props]) => {
      const element = Object.assign(window.__edsTest.defaultElement(type, 20, 20), props);
      const prims = window.__edsTest.geoPrims(element);
      const anchors = window.__edsTest.elementConnectionAnchors(element);
      const xs = anchors.map(anchor => anchor.x);
      return { type, primCount: prims.length, anchorCount: anchors.length, span: Math.max(...xs) - Math.min(...xs) };
    });
  });
  for (const item of result) {
...
test("ラベル入力後に別要素をクリックしてもクリック先が書き変わらない", async ({ page }) => {
  const result = await page.evaluate(() => {
    const a = { ...window.__edsTest.defaultElement("coil", 40, 60), id: "el-a", label: "CR1", tag: "CR1" };
    const b = { ...window.__edsTest.defaultElement("coil", 80, 60), id: "el-b", label: "CR2", tag: "CR2" };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [a, b] }]
    });
    window.__edsTest.selectElement("el-a");
    const input = document.querySelector('[data-bind="label"]');
    input.value = "編集中の値";
    input.dispatchEvent(new Event("input", { bubbles: true }));
    // 別要素をクリックした状況: 選択が移った後に元入力欄のchange(blur)が発火する
    window.__edsTest.selectElement("el-b");
    input.dispatchEvent(new Event("change", { bubbles: true }));
    const byId = id => window.__edsTest.state.pages[0].elements.find(item => item.id === id);
    return { aLabel: byId("el-a").label, bLabel: byId("el-b").label };
  });
  expect(result).toEqual({ aLabel: "編集中の値", bLabel: "CR2" });
});

test("端子箱は箱数と引き出し線を箱ごとに設定でき、接続点が先端/辺中央に付く", async ({ page }) => {
  const result = await page.evaluate(() => {
    const element = {
      ...window.__edsTest.defaultElement("terminalBox", 100, 50),
      id: "tb", rows: 4, h: 16,
      terminalBoxStubs: { "left-1": false, "right-2": false }
    };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [element] }]
    });
    const target = window.__edsTest.state.pages[0].elements[0];
    const anchors = window.__edsTest.elementConnectionAnchors(target).map(anchor => [anchor.x - 100, anchor.y - 50]);
    const stubLines = Array.from(document.querySelectorAll('g[data-id="tb"] line'))
      .filter(line => Number(line.getAttribute("x1")) < 0 || Number(line.getAttribute("x2")) > 22.5).length;
    return { anchors, stubLines };
  });
  // h=16・rows=4 → 各箱の中央は2/6/10/14(丸めなしで区画中央に一致)
  expect(result.anchors).toEqual([
    [-6, 2], [0, 2], [28.5, 2], [22.5, 2],
    [0, 6], [28.5, 6], [22.5, 6],
    [-6, 10], [0, 10], [22.5, 10],
    [-6, 14], [0, 14], [28.5, 14], [22.5, 14]
  ]);
  expect(result.stubLines).toBe(6);
});

test("斜め直線と寸法の当たり判定が実描画の線に沿う", async ({ page }) => {
  const result = await page.evaluate(() => {
    const line = { ...window.__edsTest.defaultElement("line", 0, 0), id: "diag" };
    line.points = [[20, 20], [60, 50]];
    const dim = { ...window.__edsTest.defaultElement("dimension", 0, 0), id: "dim" };
    dim.points = [[80, 40], [120, 40]];
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [line, dim] }]
    });
    return {
      lineHit: document.querySelector('g[data-id="diag"] path').getAttribute("d"),
      dimHit: document.querySelector('g[data-id="dim"] path').getAttribute("d")
    };
  });
  expect(result.lineHit).toBe("M 20 20 L 60 50");
  expect(result.dimHit).toContain("M 80 40 L 120 40");
  expect(result.dimHit).toContain("L 120 34");
});

test("ページのシンボル拡大ONで新規配置が1.5倍になり接続点も追従する", async ({ page }) => {
  const result = await page.evaluate(() => {
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", frameVariant: "blank", title: {}, symbolScale: true, elements: [] }]
    });
    const placed = window.__edsTest.defaultElementAtPlacement("contactNO", 100, 100);
    const anchors = window.__edsTest.elementConnectionAnchors(placed);
    const plain = window.__edsTest.defaultElement("contactNO", 100, 100);
    const plainAnchors = window.__edsTest.elementConnectionAnchors(plain);
    return {
      scale: placed.scale,
      span: Math.abs(anchors[1].x - anchors[0].x),
      plainSpan: Math.abs(plainAnchors[1].x - plainAnchors[0].x)
    };
  });
...
test("壁付・天井コンセントとJIS電力用コンセントを用途別の記号で描く", async ({ page }) => {
  const result = await page.evaluate(() => {
    ...
    const signatures = values.map(symbolVariant => {
      ...
      element.symbolVariant = symbolVariant;
      // 実アプリはデザイン切替時に本体寸法へリサイズするため、テストでも同じ寸法に合わせる
      const geo = window.__edsTest.SYMBOL_GEO[geoKeyByVariant[symbolVariant]];
      element.w = geo.w;
      element.h = geo.h;
      const prims = window.__edsTest.geoPrims(element);
    ...
    const migrated = window.__edsTest.migrateProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", title: {}, elements: [
        { id: "sc1", type: "outlet", x: 100, y: 50, w: 10, h: 14, symbolVariant: "box" },
  ...
  await expect(paletteOutlet).toHaveCount(1);
  ...
  })).toEqual({ variant: "standard", label: "" });
});

test("端子台・ユニット列の端子記号と機器箱の端子文字サイズ", async ({ page }) => {
  const result = await page.evaluate(() => {
    const strip = {
      ...window.__edsTest.defaultElement("terminalStrip", 40, 30),
      id: "ts", count: 4, h: 38,
      // 端子記号の検証に集中するためCOMポート(既定=あり)は無効化
      comTerminal: "",
      stripTerminalMarks: { "left-1": false, "right-2": true }
    };
    const unit = {
      ...window.__edsTest.defaultElement("plcBlock", 120, 30),
      id: "pu", plcStyle: "unit", rows: 3, w: 10, h: 30, pitch: 10,
      terminalMarkRight: true, pinText: "Y0\nY1\nY2"
    };
    const dev = {
      ...window.__edsTest.defaultElement("deviceBox", 180, 30),
      id: "dv", pins: 2, pinFontSize: 3, contentFontSize: 2.6,
      label: "DEV", pinText: "1\n2", rightPinText: ""
    };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [strip, unit, dev] }]
    });
    // 端子記号=機器箱の端子円と同一仕様: r0.55白抜き小円を本体辺上へ描くだけ(破線などの付加線はなし)
    const markCount = selector => Array.from(document.querySelectorAll(selector + " circle"))
      .filter(node => Math.abs(Number(node.getAttribute("r")) - 0.55) < 0.01).length;
    const dashCount = selector => Array.from(document.querySelectorAll(selector + " line"))
      .filter(node => node.getAttribute("stroke-dasharray")).length;
    const stripMarkOnEdge = Array.from(document.querySelectorAll('g[data-id="ts"] circle'))
      .filter(node => Math.abs(Number(node.getAttribute("r")) - 0.55) < 0.01)
      .every(node => Number(node.getAttribute("cx")) === 0 || Number(node.getAttribute("cx")) === 10);
    const pinSizes = Array.from(document.querySelectorAll('g[data-id="dv"] text[data-content-exempt="true"]'))
      .map(node => node.getAttribute("font-size"));
    const labelNode = Array.from(document.querySelectorAll('g[data-id="dv"] text')).find(node => node.textContent === "DEV");
    const byId = id => window.__edsTest.state.pages[0].elements.find(item => item.id === id);
    const stripAnchors = window.__edsTest.elementConnectionAnchors(byId("ts")).map(anchor => [anchor.x, anchor.y]);
    const unitAnchors = window.__edsTest.elementConnectionAnchors(byId("pu")).map(anchor => [anchor.x, anchor.y]);
    return {
      stripMarks: markCount('g[data-id="ts"]'),
      stripDashes: dashCount('g[data-id="ts"]'),
      stripMarkOnEdge,
      unitMarks: markCount('g[data-id="pu"]'),
      unitDashes: dashCount('g[data-id="pu"]'),
      stripAnchors,
      unitAnchors,
      pinSizes,
      labelSize: labelNode?.getAttribute("font-size")
    };
  });
  // 端子台: パレット既定=左ON(4端子) - left-1 OFF + right-2 ON = 4個。破線などの付加線は出ない
  expect(result.stripMarks).toBe(4);
  expect(result.stripDashes).toBe(0);
  expect(result.stripMarkOnEdge).toBe(true);
  expect(result.unitMarks).toBe(3);
  expect(result.unitDashes).toBe(0);
  // 配線接続点は端子(小円中心=本体辺上)のみ。端子記号がある行は中央ノードへ接続できない
  expect(result.stripAnchors).toEqual(expect.arrayContaining([
    [40, 34.75], [50, 53.75],
    [45, 44.25]
  ]));
  expect(result.stripAnchors).toEqual(expect.not.arrayContaining([[35, 34.75]]));
  expect(result.stripAnchors).toEqual(expect.not.arrayContaining([[45, 34.75]]));
  expect(result.stripAnchors).toEqual(expect.not.arrayContaining([[45, 53.75]]));
  expect(result.unitAnchors).toEqual(expect.arrayContaining([
    [130, 35], [130, 55]
  ]));
...
test("ラベル文字サイズ入力は文字スタイルの文字サイズへ一本化されている", async ({ page }) => {
  const result = await page.evaluate(() => {
    const coil = { ...window.__edsTest.defaultElement("coil", 40, 40), id: "cl", label: "CR1", tag: "CR1" };
    const line = { ...window.__edsTest.defaultElement("line", 0, 0), id: "ln", label: "L1" };
    line.points = [[80, 40], [110, 40]];
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [coil, line] }]
    });
    const counts = {};
    ["cl", "ln"].forEach(id => {
      window.__edsTest.selectElement(id);
      counts[id] = {
        labelSizeInputs: document.querySelectorAll('[data-bind="labelFontSize"]').length,
        styleSizeInputs: document.querySelectorAll('[data-bind="contentFontSize"]').length
      };
    });
    return counts;
  });
  expect(result.cl).toEqual({ labelSizeInputs: 0, styleSizeInputs: 1 });
  expect(result.ln).toEqual({ labelSizeInputs: 0, styleSizeInputs: 1 });
});

test("フォトダイオード・ポテンショメータ・インバータ箱を作画できる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const make = (type, props) => Object.assign(window.__edsTest.defaultElement(type, 20, 20), props);
    const photo = make("diode", { symbolVariant: "photo" });
    const led = make("diode", { symbolVariant: "led" });
    const pot = make("resistor", { resistorStyle: "potentiometer" });
    const inv = make("powerSupply", { symbolVariant: "dcac", w: 35, h: 25 });
    const anchorsOf = element => window.__edsTest.elementConnectionAnchors(element)
      .map(anchor => [Math.round((anchor.x - 20) * 1000) / 1000, Math.round((anchor.y - 20) * 1000) / 1000]);
    return {
      photoPrims: JSON.stringify(window.__edsTest.geoPrims(photo)),
      ledPrims: JSON.stringify(window.__edsTest.geoPrims(led)),
      potAnchors: anchorsOf(pot),
      invPrims: window.__edsTest.geoPrims(inv).length,
      invAnchors: anchorsOf(inv)
    };
  });
  expect(result.photoPrims).not.toBe(result.ledPrims);
  expect(result.potAnchors).toEqual([[0, 1.3], [10, 1.3], [5, -3.7]]);
  expect(result.invPrims).toBeGreaterThan(4);
  expect(result.invAnchors).toEqual([[0, 12.5], [35, 12.5]]);
});

test("右パネルはページ/部品タブで切り替わり選択状態に追従する", async ({ page }) => {
  await page.evaluate(() => {
    const coil = { ...window.__edsTest.defaultElement("coil", 40, 40), id: "cl", label: "CR1", tag: "CR1" };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [coil] }]
    });
  });
  await expect(page.locator("#inspectorPageTab")).toBeVisible();
  await expect(page.locator("#inspectorTabComponent")).toBeDisabled();
  await page.evaluate(() => window.__edsTest.selectElement("cl"));
  await expect(page.locator("#selectionPanel")).toBeVisible();
  await expect(page.locator("#inspectorPageTab")).toBeHidden();
  await page.locator("#inspectorTabPage").click();
  await expect(page.locator("#inspectorPageTab")).toBeVisible();
...
test("端子の一括編集ダイアログで端子箱をまとめて変更できる", async ({ page }) => {
  await page.evaluate(() => {
    const tb = { ...window.__edsTest.defaultElement("terminalBox", 60, 40), id: "tb", rows: 3, boxLabels: "R\nS\nT" };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [tb] }]
    });
    window.__edsTest.selectElement("tb");
  });
  await page.locator("#bulkTerminalEditBtn").click();
  await expect(page.locator("#bulkTerminalApply")).toBeVisible();
  await page.locator('[data-bulk-all="leftStub:off"]').click();
  await page.locator('[data-bulk-cell="name:1"]').fill("S2");
  await page.locator("#bulkTerminalApply").click();
  const after = await page.evaluate(() => {
    const tb = window.__edsTest.state.pages[0].elements[0];
    return { boxLabels: tb.boxLabels, stubs: tb.terminalBoxStubs };
  });
  expect(after.boxLabels).toBe("R\nS2\nT");
  expect(after.stubs["left-0"]).toBe(false);
  expect(after.stubs["left-2"]).toBe(false);
  expect(after.stubs["right-1"]).toBe(true);
});

test("同一フィールドの連続編集は履歴1件へ合体する", async ({ page }) => {
  const counts = await page.evaluate(() => {
    ...
    window.__edsTest.selectElement("cl");
    const before = window.__edsTest.historyDiagnostics().count;
    const rotation = document.querySelector('#selectionPanel [data-bind="rotation"]');
    [15, 30, 45].forEach(value => {
      rotation.value = String(value);
      rotation.dispatchEvent(new Event("change", { bubbles: true }));
    });
    const merged = window.__edsTest.historyDiagnostics().count;
    const label = document.querySelector('#selectionPanel [data-bind="label"]');
    label.value = "CR9";
    label.dispatchEvent(new Event("change", { bubbles: true }));
    const after = window.__edsTest.historyDiagnostics().count;
    return { before, merged, after, rotation: window.__edsTest.state.pages[0].elements[0].rotation };
  });
  expect(counts.merged - counts.before).toBe(1);
  expect(counts.after - counts.merged).toBe(1);
  expect(counts.rotation).toBe(45);
});

test("パレットの☆でお気に入りへ固定できる", async ({ page }) => {
  const favored = await page.evaluate(() => {
    document.querySelector('#paletteGrid .palette-item[data-type="coil"] .palette-fav').click();
    return (window.__edsTest.state.settings.favoriteTools || []).includes("coil");
  });
  expect(favored).toBe(true);
  const activeStars = await page.evaluate(() =>
    document.querySelectorAll('#paletteGrid .palette-item[data-type="coil"] .palette-fav.active').length);
  expect(activeStars).toBeGreaterThan(0);
});

test("クイックプロパティにデザイン切替と詳細ボタンが出る", async ({ page }) => {
  await page.evaluate(() => {
    const coil = { ...window.__edsTest.defaultElement("coil", 40, 40), id: "cl", label: "CR1", tag: "CR1" };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
  ...
  await page.keyboard.press("q");
  await expect(page.locator("#quickVariant")).toBeVisible();
  await page.locator("#quickVariant").selectOption("box");
  await page.locator("#quickApply").click();
  expect(await page.evaluate(() => window.__edsTest.state.pages[0].elements[0].symbolVariant)).toBe("box");
  await page.keyboard.press("q");
  await page.locator("#quickOpenDetail").click();
  await expect(page.locator("#selectionPanel")).toBeVisible();
});
...
test("PLCユニット列のCOM端子とテンプレートのCOM配線", async ({ page }) => {
  ...
  const output = await page.evaluate(() => {
    ...
    const com = current.elements.find(element => element.type === "wire" && Array.isArray(element.points)
      && element.points[0][1] === 77.5 && element.points[1][1] === 77.5);
    const zero = current.elements.find(element => element.type === "wire" && Array.isArray(element.points)
...
test("端子台のCOMポートは標準装備で位置は端子記号の逆側が自動既定", async ({ page }) => {
  const result = await page.evaluate(() => {
    const auto = { ...window.__edsTest.defaultElement("terminalStrip", 40, 30), id: "tsA", count: 2, h: 28.5 };
    const flipped = { ...window.__edsTest.defaultElement("terminalStrip", 100, 30), id: "tsB", count: 2, h: 28.5, terminalMarkLeft: false, terminalMarkRight: true };
    const hiddenMark = { ...window.__edsTest.defaultElement("terminalStrip", 160, 30), id: "tsC", count: 2, h: 28.5, stripTerminalMarks: { com: false } };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [auto, flipped, hiddenMark] }]
    });
    const byId = id => window.__edsTest.state.pages[0].elements.find(item => item.id === id);
    const anchorsOf = (id, ox) => window.__edsTest.elementConnectionAnchors(byId(id)).map(anchor => [anchor.x - ox, anchor.y - 30]);
    const comTexts = id => Array.from(document.querySelectorAll(`g[data-id="${id}"] text`)).filter(node => node.textContent === "COM").length;
    return {
      autoDefault: byId("tsA").comTerminal,
      autoCom: comTexts("tsA"),
      autoAnchors: anchorsOf("tsA", 40),
      flippedAnchors: anchorsOf("tsB", 100),
      hiddenAnchors: anchorsOf("tsC", 160)
    };
  });
  // パレット既定: COMあり(auto)。既定の端子記号=左なのでCOM端子記号は右(w=10)側、y=2行分(19)+4.75
  expect(result.autoDefault).toBe("auto");
  expect(result.autoCom).toBe(1);
  expect(result.autoAnchors).toEqual(expect.arrayContaining([[10, 23.75]]));
  // 端子記号を右へ変えるとCOMは自動で左(0)側へ
  expect(result.flippedAnchors).toEqual(expect.arrayContaining([[0, 23.75]]));
  // COMの端子記号を非表示にするとCOM円中心(5)が接続点
  expect(result.hiddenAnchors).toEqual(expect.arrayContaining([[5, 23.75]]));
  expect(result.hiddenAnchors).toEqual(expect.not.arrayContaining([[10, 23.75]]));
});

test("図枠の既定は基本(右下表題欄)でテンプレートページも同じ", async ({ page }) => {
  const frames = await page.evaluate(() => window.__edsTest.state.pages.map(item => item.frameVariant));
  expect(frames).toEqual(["compact"]);
  await page.locator("#templateMenu").selectOption("plcInput");
  await page.locator("#plcInBuildBtn").click();
  const templateFrame = await page.evaluate(() => {
    const current = window.__edsTest.state.pages.find(item => item.id === window.__edsTest.state.activePageId);
    return current.frameVariant;
  });
  expect(templateFrame).toBe("compact");
});

test("シンボルラベルの既定位置は上側に統一されている", async ({ page }) => {
  const below = await page.evaluate(() => {
    const list = [];
    Object.entries(window.__edsTest.SYMBOL_GEO).forEach(([key, geo]) => {
      (geo.prims || []).forEach(prim => {
        if (prim.t === "text" && prim.field === "label" && prim.p[1] > geo.h) list.push(key);
      });
    });
    return list;
  });
...
test("PLC入力回路は各行が個別にP1へ接続されCOMもレール接続点を持つ", async ({ page }) => {
  await page.locator("#templateMenu").selectOption("plcInput");
  await page.locator("#plcInCount").fill("3");
  await page.locator("#plcInBuildBtn").click();
  const result = await page.evaluate(() => {
    const current = window.__edsTest.state.pages.find(item => item.id === window.__edsTest.state.activePageId);
    const junctions = current.elements.filter(element => element.type === "junction").map(element => [element.x, element.y]);
  ...
  expect(result.feeder).toBe(false);
  expect(result.rowWires).toBe(3);
  expect(result.junctions).toEqual(expect.arrayContaining([[32.5, 37.5], [32.5, 47.5], [32.5, 57.5], [147.5, 67.5]]));
...
test("端子台の出力カードモードはa接点+デバイス名+0Vポートで描かれる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const strip = {
      ...window.__edsTest.defaultElement("terminalStrip", 40, 30),
      id: "out", count: 2, h: 38, cardMode: "output",
      terminalLabels: "Y0\nY1"
    };
    window.__edsTest.installProjectData({
      schemaVersion: 4,
      activePageId: "p",
      pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [strip] }]
    });
    const target = window.__edsTest.state.pages[0].elements[0];
    const group = document.querySelector('g[data-id="out"]');
    const texts = Array.from(group.querySelectorAll("text")).map(node => node.textContent);
    const contactBars = Array.from(group.querySelectorAll("rect"))
      .filter(node => Number(node.getAttribute("width")) === 0.3).length;
    const anchors = window.__edsTest.elementConnectionAnchors(target)
      .map(anchor => [Math.round((anchor.x - 40) * 100) / 100, Math.round((anchor.y - 30) * 100) / 100]);
    return { texts, contactBars, anchors };
  });
  // 出力カード: 各ポート=a接点(縦バー2本)+上にデバイス名。COMと0Vの行も出る
  expect(result.contactBars).toBe(4);
  expect(result.texts).toEqual(expect.arrayContaining(["Y0", "Y1", "COM", "0V"]));
  // 既定: 端子=左(0)、COM=右(w=10)、0V=左(0)。h=38・4スロット→ピッチ9.5
...
test("光電・マット・リードスイッチは幅15mm前後へ縮小され接続点は2点のまま", async ({ page }) => {
  const result = await page.evaluate(() => {
    const cases = {
      photoelectricSwitch: ["throughBeam", "general", "convergent", "colorMark"],
      matSwitch: ["standard", "safety"],
      reedSwitch: ["no", "cylinder"]
    };
    const report = { defaults: {}, variants: [] };
    ["photoelectricSwitch", "matSwitch", "reedSwitch"].forEach(type => {
      const base = window.__edsTest.defaultElement(type, 20, 20);
      report.defaults[type] = [base.w, base.h];
    });
    Object.entries(cases).forEach(([type, variants]) => {
      variants.forEach(symbolVariant => {
        const element = window.__edsTest.defaultElement(type, 20, 20);
        element.symbolVariant = symbolVariant;
        const anchors = window.__edsTest.elementConnectionAnchors(element);
        const xs = anchors.map(anchor => anchor.x);
        report.variants.push({ anchorCount: anchors.length, span: Math.round((Math.max(...xs) - Math.min(...xs)) * 100) / 100 });
      });
    });
    return report;
  });
  // 既定寸法は元の25×14から0.6倍の15×8.4へ縮小
  expect(result.defaults).toEqual({
    photoelectricSwitch: [15, 8.4],
    matSwitch: [15, 8.4],
    reedSwitch: [15, 8.4]
  });
  // 全バリアントで接続ピン間隔15mm(2.5mmグリッド整合)・接続点2点を維持
  expect(result.variants.every(item => item.anchorCount === 2)).toBe(true);
  expect(result.variants.every(item => item.span === 15)).toBe(true);
});

test("デザイン選択肢を持つ全種別で右パネルにシンボルデザイン選択が出る", async ({ page }) => {
  const result = await page.evaluate(() => {
    const options = window.__edsTest.SYMBOL_VARIANT_OPTIONS;
    // pushButtonEmergencyはpushButton種別(pbStyle=emergency)で出るため、配置typeへ読み替える。
    const keyToType = { pushButtonEmergency: "pushButton" };
    const missing = [];
    const mismatched = [];
    Object.keys(options).forEach(key => {
      const type = keyToType[key] || key;
      let base;
      try { base = window.__edsTest.defaultElement(type, 30, 30); }
      catch (_) { missing.push(key + "(defaultElement失敗)"); return; }
      const element = { ...base, id: "chk" };
      if (key === "pushButtonEmergency") element.pbStyle = "emergency";
      window.__edsTest.installProjectData({
        schemaVersion: 4, activePageId: "p",
        pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [element] }]
      });
      window.__edsTest.selectElement("chk");
      const sel = document.querySelector('#selectionPanel select[data-bind="symbolVariant"]');
      if (!sel) { missing.push(key); return; }
      const optionCount = sel.querySelectorAll("option").length;
      // 現在値が定義に無い等でオプションが1つ増える場合を許容し、下限のみ確認。
      if (optionCount < options[key].length) mismatched.push(`${key}:${optionCount}/${options[key].length}`);
    });
...
test("押しボタンa/b接点は上部の押しキャップ横棒を持たず縦棒と接点バーだけになる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const make = variant => {
      const element = window.__edsTest.defaultElement("pushButton", 20, 20);
      element.symbolVariant = variant;
      element.pbStyle = "momentary";
      return window.__edsTest.geoPrims(element);
    };
    const lineAt = (prims, y) => prims.some(prim => prim.t === "line"
      && Math.abs(prim.p[1] - y) < 0.001 && Math.abs(prim.p[3] - y) < 0.001);
    const vStem = prims => prims.some(prim => prim.t === "line" && Math.abs(prim.p[0] - prim.p[2]) < 0.001);
    const a = make("standard");
    const b = make("nc");
    return {
      // 一番上の横棒(y=1.0の水平線)は削除済み
      aTopCap: lineAt(a, 1.0),
      bTopCap: lineAt(b, 1.0),
      // 縦棒と接点の横棒は残る(a=y2.2, b=y4.1)
      aStem: vStem(a), aContactBar: lineAt(a, 2.2),
      bStem: vStem(b), bContactBar: lineAt(b, 4.1)
    };
  });
  expect(result.aTopCap).toBe(false);
  expect(result.bTopCap).toBe(false);
  expect(result.aStem).toBe(true);
  expect(result.aContactBar).toBe(true);
  expect(result.bStem).toBe(true);
  expect(result.bContactBar).toBe(true);
});

test("geoシンボルのラベル文字サイズは文字スタイルで変更でき構造記号は不変", async ({ page }) => {
  const result = await page.evaluate(() => {
    const check = (type, variant, label) => {
      const el = { ...window.__edsTest.defaultElement(type, 30, 30), id: "x", symbolVariant: variant, label, contentFontSize: 4.2, contentFontBold: true };
      window.__edsTest.installProjectData({
        schemaVersion: 4, activePageId: "p",
        pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", frameVariant: "blank", title: {}, elements: [el] }]
      });
      const g = document.querySelector('g[data-id="x"]');
      const labelNode = Array.from(g.querySelectorAll("text")).find(n => n.textContent === label);
      const structural = Array.from(g.querySelectorAll("text")).filter(n => n.textContent !== label);
      const dxf = window.__edsTest.buildDxf(window.__edsTest.state.pages[0]);
      return {
        labelSize: labelNode && labelNode.getAttribute("font-size"),
        labelBold: labelNode && labelNode.getAttribute("font-weight"),
        structuralSizes: structural.map(n => n.getAttribute("font-size")),
        dxfHasBigLabel: dxf.includes("\n40\n4.2\n")
      };
    };
    return {
      photo: check("photoelectricSwitch", "throughBeam", "PH1"),
      contact: check("contactNO", "standard", "CR1"),
      colorMark: check("photoelectricSwitch", "colorMark", "PH2")
    };
  });
  // 光電・接点ともラベルがcontentFontSize=4.2・太字で反映され、DXFにも出る
  expect(result.photo.labelSize).toBe("4.2");
  expect(result.photo.labelBold).toBe("700");
  expect(result.photo.dxfHasBigLabel).toBe(true);
  expect(result.contact.labelSize).toBe("4.2");
  expect(result.contact.dxfHasBigLabel).toBe(true);
  // colorMarkの構造記号"C"はラベルサイズ変更の影響を受けない(4.2にはならない)
  expect(result.colorMark.labelSize).toBe("4.2");
  expect(result.colorMark.structuralSizes.every(size => size !== "4.2")).toBe(true);
});

test("ブザーは他部品と同等サイズへ縮小され図形が枠内に収まる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const base = window.__edsTest.defaultElement("buzzer", 40, 40);
    const check = variant => {
      const el = { ...window.__edsTest.defaultElement("buzzer", 40, 40), id: "bz", buzzerVariant: variant, label: "BZ" };
      window.__edsTest.installProjectData({
        schemaVersion: 4, activePageId: "p",
        pages: [{ id: "p", name: "P1", size: "A4", orientation: "portrait", frameVariant: "blank", title: {}, elements: [el] }]
      });
      const g = document.querySelector('g[data-id="bz"]');
      // ラベル以外の図形(line/path/circle)のバウンディングを集計し、枠[0..w]x[-margin..h]に収まるか確認
      const w = el.w, h = el.h;
      let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
      g.querySelectorAll("line,circle,path").forEach(node => {
        const bb = node.getBBox();
        minX = Math.min(minX, bb.x); maxX = Math.max(maxX, bb.x + bb.width);
        minY = Math.min(minY, bb.y); maxY = Math.max(maxY, bb.y + bb.height);
      });
      const anchors = window.__edsTest.elementConnectionAnchors(el);
      return { w, h, minX: Math.round(minX * 10) / 10, maxX: Math.round(maxX * 10) / 10, minY: Math.round(minY * 10) / 10, maxY: Math.round(maxY * 10) / 10, anchors: anchors.length };
    };
    return { defaults: [base.w, base.h], horn: check("horn"), speaker: check("speaker") };
  });
  // 既定は12x6(旧15x9から縮小、bell/lampと同等スケール)
  expect(result.defaults).toEqual([12, 6]);
  // 図形は横0..w、縦は枠(0..h)内(わずかな線幅ぶんの許容0.6mm)に収まる=はみ出さない
  [result.horn, result.speaker].forEach(r => {
    expect(r.anchors).toBe(2);
    expect(r.minX).toBeGreaterThanOrEqual(-0.1);
    expect(r.maxX).toBeLessThanOrEqual(r.w + 0.1);
    expect(r.minY).toBeGreaterThanOrEqual(-0.6);
    expect(r.maxY).toBeLessThanOrEqual(r.h + 0.6);
  });
});

test("COM/0Vポートの表示名をCOM＋/COM−へ切り替えられる", async ({ page }) => {
  const result = await page.evaluate(() => {
    const build = comLabelStyle => {
      const el = { ...window.__edsTest.defaultElement("terminalStrip", 40, 25), id: "tb", count: 2, h: 38, cardMode: "output", comLabelStyle };
      window.__edsTest.installProjectData({
        schemaVersion: 4, activePageId: "pg",
        pages: [{ id: "pg", name: "P1", size: "A4", orientation: "portrait", title: {}, elements: [el] }]
      });
      const texts = Array.from(document.querySelectorAll('g[data-id="tb"] text')).map(n => n.textContent);
      const dxf = window.__edsTest.buildDxf(window.__edsTest.state.pages[0]);
      return { texts, dxf };
    };
    const normal = build(undefined);
    const pm = build("plusMinus");
    return {
      normalHasCom: normal.texts.includes("COM") && normal.texts.includes("0V"),
      pmHasPlusMinus: pm.texts.includes("COM+") && pm.texts.includes("COM-"),
      pmNoOldLabels: !pm.texts.includes("COM") && !pm.texts.includes("0V"),
      dxfPm: pm.dxf.includes("COM+") && pm.dxf.includes("COM-")
    };
  });
  expect(result.normalHasCom).toBe(true);
  expect(result.pmHasPlusMinus).toBe(true);
  expect(result.pmNoOldLabels).toBe(true);
  expect(result.dxfPm).toBe(true);
});

test("ブザー系はJIS C 0617準拠(08-10-06半円/08-10-10反転半円/08-10-09直角三角形)で枠内に収まる", async ({ page }) => {
  const result = await page.evaluate(() => {
  ...
  expect(result.jis0806.arcs).toEqual([[180, 360]]);
  expect(result.jis0806.vlines).toBe(2);
  // ブザー(08-10-10): 08-10-06の半円を上下反転(0→180の下ふくらみ)+端子線2本
  expect(result.jis0810.arcs).toEqual([[0, 180]]);
  expect(result.jis0810.vlines).toBe(2);
  // サイレン(08-10-09): 直角三角形(pline)+端子線2本、弧なし
...
test("全パレット部品が配置でき、ブザー等の全バリアントが選択できる", async ({ page }) => {
  const result = await page.evaluate(() => {
    ...
    const cfgs = [
      ["buzzer", "buzzerVariant", ["jis0806", "jis0810", "siren", "horn", "speaker", "sound", "piezo"]],
...
test("グリッド表示をスナップとは独立して切り替えて保存できる", async ({ page }) => {
  ...
  await page.locator("#gridVisibility").selectOption("off");
  await expect(gridLayers).toHaveCount(0);
  expect(await page.evaluate(() => ({
    showGrid: window.__edsTest.state.settings.showGrid,
    snap: window.__edsTest.state.settings.snap
  }))).toEqual({ showGrid: false, snap: true });
...
test("JISブザー08-10-06/08-10-10は弦の内側から下ろした端子線をピッチ2.5mmの接続点にする", async ({ page }) => {
  const result = await page.evaluate(() => {
    const api = window.__edsTest;
    const probe = variant => {
      const buzzer = { ...api.defaultElement("buzzer", 40, 50), id: "bz-" + variant, buzzerVariant: variant };
      const drawingPage = api.createPage({ name: "JISブザー" });
      drawingPage.elements = [buzzer];
      api.installProjectData({ ...api.state, pages: [drawingPage], activePageId: drawingPage.id });
      const prims = api.geoPrims(buzzer);
      return {
        // anchorsはページ座標、prims/SVG属性は要素ローカル座標。比較用に要素原点も返す。
        origin: [buzzer.x, buzzer.y],
        anchors: api.elementConnectionAnchors(buzzer).map(anchor => [anchor.x, anchor.y]),
        arc: prims.find(prim => prim.t === "arc").p,
        lines: Array.from(document.querySelectorAll('[data-id="bz-' + variant + '"] line')).map(line => ({
          x1: Number(line.getAttribute("x1")), y1: Number(line.getAttribute("y1")),
          x2: Number(line.getAttribute("x2")), y2: Number(line.getAttribute("y2"))
        }))
      };
    };
    return { a: probe("jis0806"), b: probe("jis0810") };
  });

  for (const [name, r] of Object.entries(result)) {
    expect(r.anchors, name).toHaveLength(2);
    // 2点は同じ高さ・ピッチ2.5mm(グリッド整合)で、記号本体より下にある
    expect(r.anchors[0][1], name).toBe(r.anchors[1][1]);
    expect(Math.abs(r.anchors[1][0] - r.anchors[0][0]), name).toBeCloseTo(2.5, 6);
    expect(r.anchors[0][1], name).toBeGreaterThan(50);
    const terminalLines = r.lines.filter(line => line.x1 === line.x2 && line.y2 > line.y1);
    const chord = r.lines.find(line => line.y1 === line.y2 && line.x2 > line.x1);
    // 端子線は2本、いずれも弦の両端より内側から出る
    expect(terminalLines, name).toHaveLength(2);
    expect(terminalLines[0].x1, name).toBeGreaterThan(chord.x1);
    expect(terminalLines[1].x1, name).toBeLessThan(chord.x2);
    // 端子線の下端は接続点と一致する(線の切れがない)
    terminalLines.forEach(line => expect(line.y2 + r.origin[1], name).toBeCloseTo(r.anchors[0][1], 6));
    terminalLines.forEach((line, index) => expect(line.x1 + r.origin[0], name).toBeCloseTo(r.anchors[index][0], 6));
  }
  // 08-10-06は上ふくらみ(180→360)で端子線は弦から、08-10-10は下ふくらみ(0→180)で端子線は弧から下ろす
  expect(result.a.arc.slice(3)).toEqual([180, 360]);
  expect(result.b.arc.slice(3)).toEqual([0, 180]);
  const [cx, cy, radius] = result.b.arc;
  result.b.lines.filter(line => line.x1 === line.x2 && line.y2 > line.y1).forEach(line => {
    // 端子線の上端が弧の上に載っていること(隙間も突き抜けもない): 中心からの距離=半径
    const distance = Math.hypot(line.x1 - cx, line.y1 - cy);
    ...
    cases.forEach(([type, variants]) => {
      variants.forEach(variant => {
        ...
        prims.forEach(prim => {
          ...
          let xs, ys;
          if (prim.pts) { xs = prim.pts.map(point => point[0]); ys = prim.pts.map(point => point[1]); }
          else if (prim.t === "circle" || prim.t === "arc") {
            xs = [prim.p[0] - prim.p[2], prim.p[0] + prim.p[2]];
            ys = [prim.p[1] - prim.p[2], prim.p[1] + prim.p[2]];
          } else if (prim.t === "rect") {
            xs = [prim.p[0], prim.p[0] + prim.p[2]]; ys = [prim.p[1], prim.p[1] + prim.p[3]];
          } else { xs = [prim.p[0], prim.p[2]]; ys = [prim.p[1], prim.p[3]]; }
          minX = Math.min(minX, ...xs); maxX = Math.max(maxX, ...xs);
          minY = Math.min(minY, ...ys); maxY = Math.max(maxY, ...ys);
        });
        rows.push({
          type, variant, w: element.w, h: element.h, primCount: prims.length,
          box: [minX, minY, maxX, maxY],
          // 接続点はページ座標で返るので、要素原点(20,20)を引いて要素ローカル座標で比較する
