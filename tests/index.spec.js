...
test("端子台・ユニット列の端子記号と機器箱の端子文字サイズ", async ({ page }) => {
  const result = await page.evaluate(() => {
    ...
    // 端子記号=単体端子・機器箱と同径のr0.55白抜き小円(本体辺上)+盤外側への破線スタブ
    ...
    const dashCount = selector => Array.from(document.querySelectorAll(selector + " line"))
      .filter(node => node.getAttribute("stroke-dasharray") === "3 2").length;
  ...
  // 端子台: パレット既定=左ON(4端子) - left-1 OFF + right-2 ON = 4個(破線スタブも同数)
  expect(result.stripMarks).toBe(4);
  expect(result.stripDashes).toBe(4);
  expect(result.stripMarkOnEdge).toBe(true);
  expect(result.unitMarks).toBe(3);
  expect(result.unitDashes).toBe(3);
  // 端子記号は配線接続点になる: 小円中心(辺上)と破線スタブ先端(5mm外側)の両方(端子台はピッチ9.5=h38/4)
